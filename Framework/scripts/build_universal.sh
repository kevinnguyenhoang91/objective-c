...
OBJROOT_SDK="${OBJROOT}_${sdk}"
BUILD_ROOT_SDK="${BUILD_ROOT}_${sdk}"
SYMROOT_SDK="${SYMROOT}_${sdk}"
BUILD_DIR_SDK="${BUILD_DIR}_${sdk}"
...
echo "xcrun --no-cache xcodebuild -project \"${PROJECT_FILE_PATH}\" -target \"${FRAMEWORK_NAME}\" -configuration \"${FRAMEWORKS_BUILD_CONFIGURATION}\" -sdk \"${sdk}\" BUILD_DIR=\"${BUILD_DIR}\" OBJROOT=\"${OBJROOT}\" BUILD_ROOT=\"${BUILD_ROOT}\" SYMROOT=\"${SYMROOT}\" ARCHS=\"${FRAMEWORKS_TARGET_ARCHITECTURES}\" VALID_ARCHS=\"${FRAMEWORKS_TARGET_ARCHITECTURES}\" ONLY_ACTIVE_ARCH=NO ENABLE_BITCODE=${PNENABLE_BITCODE} $ACTION"
    xcrun --no-cache xcodebuild -project "${PROJECT_FILE_PATH}" -target "${FRAMEWORK_NAME}" -configuration "${FRAMEWORKS_BUILD_CONFIGURATION}" -sdk "${sdk}" BUILD_DIR="${BUILD_DIR_SDK}" OBJROOT="${OBJROOT_SDK}" BUILD_ROOT="${BUILD_ROOT_SDK}" SYMROOT="${SYMROOT_SDK}" ARCHS="${FRAMEWORKS_TARGET_ARCHITECTURES}" VALID_ARCHS="${FRAMEWORKS_TARGET_ARCHITECTURES}" ONLY_ACTIVE_ARCH=NO ENABLE_BITCODE=$PNENABLE_BITCODE $ACTION
...
do
    BUILD_DIR_SDK="${BUILD_DIR}_${sdk}"
    if [[ $sdk =~ (simulator) ]]; then
        SIMULATOR_ARTIFACTS_PATH="${BUILD_DIR_SDK}/${FRAMEWORKS_BUILD_CONFIGURATION}-${sdk}"
    else
        OS_ARTIFACTS_PATH="${BUILD_DIR_SDK}/${FRAMEWORKS_BUILD_CONFIGURATION}-${sdk}"
    fi
done
